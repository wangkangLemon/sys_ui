                ...
                <el-form  class="testing-set"  >
                    <el-form class="cate" label-width="120px" :model="form"  :rules="rules" ref="cate">
                        ...
                        <el-form-item label="答案选项组" v-show="qtype=='A4'">
                            <h5>请在正确答案前面打勾</h5>
                            <div class="multy-choose-item" v-for="(option,indexOption) in anweroption" :key="indexOption">
                                ...
                                <el-button  type="text" @click="anweroption.splice(indexOption, 1)">
                                    <i>删除</i>
                                </el-button>
                            </div>
                            <div class="multy-choose-item">
                                <el-button  type="text" @click="addMoreTestingOption(anweroption)">添加更多选项</el-button>
                            </div>
                        </el-form-item>
                        <hr>
                    </el-form>
                    <el-form label-width="120px" v-for="(item,index) in fetchTesting" :key="index" :model="item"  ref="test">
                        ...
                        <!--A4单选|多选的选项部分-->
                        <el-form-item label="选项" v-if="qtype=='A4'" v-for="(option,indexOption) in anweroption" :key="indexOption">
                            <el-checkbox class="radio" :disabled="!item.editable" v-model="option.correct" :label="option.description" :true-label="1" :false-label="0">
                                <i>{{option.description}}</i>
                            </el-checkbox>
                        </el-form-item>
                         <!--A4单选|多选的选项部分-->
                        <optionItemA4 :options="anweroption" :item="item" >
        ...
        handleSubmitTesting() {
            ...
            this.$refs.cate.validate((valid) => {
                ...
                for (let i = 0; i < requestParam.length, item = requestParam[i]; i++) {
                    ...
                    requestParam["options"]=item.options
                // requestParam["qtype"]=this.qtype
                // requestParam["title"]=this.form.title
               
                }
                console.log(requestParam)
                console.log(typeof(requestParam))
                // console.log(typeof(this.qtype),typeof(this.form.title))
                requestParam["options"]=JSON.stringify(requestParam) //A4 型添加到外层结构
                requestParam["qtype"]="A4"
                requestParam["title"]="题干"
                console.log(requestParam)
