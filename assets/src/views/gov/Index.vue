...
<template>
    <article id="gov-index-container">
        <section class="manage-container">
            <el-button type="primary" icon="plus" @click="$router.push({ name:'gov-add',params:{sys_type:'add'}})">
                <i>添加管理员</i>
            </el-button>
        </section>
        ...
        <el-table class="data-table" v-loading="loadingData" :data="tableData" :fit="true" @select="selectRow" @select-all="selectRow" border>
            ...
            <el-table-column min-width="200" prop="category" label="部门分类" >
            </el-table-column>
            <el-table-column min-width="200" prop="name" label="上级部门" >
            </el-table-column>
            <el-table-column min-width="200" prop="name" label="省" >
            </el-table-column>
            <el-table-column min-width="200" prop="name" label="市" >
            </el-table-column>
            <el-table-column min-width="200" prop="name" label="市" >
            </el-table-column>
            <el-table-column min-width="200" prop="name" label="市" >
            ...
            <el-table-column fixed="right" width="207" label="操作">
                <template scope="scope">
                    ...
                    <el-button @click="$router.push({name: 'gov-edit', params: {sysInfo: scope.row, sys_id: scope.row.id}})" type="text" size="small">详情
                        <!--a-->
                    </el-button>
                    <el-button @click="$router.push({name: 'gov-edit', params: {sysInfo: scope.row, sys_id: scope.row.id, sys_type:'edit'}})" type="text" size="small">编辑
                        <!--a-->
                    </el-button>
...
export default {
    ...
    methods: {
        ...
        fetchData(val) {
            return govService.fetchData(this.fetchParam).then((ret) => {
        ...
        search(val){
               return govService.search(this.fetchParam).then((ret) => {
            })
        },      
        // 单行被选中
        selectRow(selection) {
        ...
        offline(index, row) {
            if(row.deleted == 0){
                xmview.showDialog(`你将要禁用管理员 <span style="color:red">${row.name}</span> 确认吗?`, () => {
                    row.disabled = 1
                    govService.offline(row).then((ret) => {
        ...
        online(index, row) {
            if(row.deleted == 0){
                xmview.showDialog(`你将要启用管理员<span style="color:red">${row.name}</span> 确认吗?`, () => {
                    govService.online(row.id).then((ret) => {
                        row.disabled = 0
                    })
                })
            }else{
        ...
        delMulti() {
            xmview.showDialog(`你将要删除选中的项目，操作不可恢复确认吗?`, () => {
                govService.deleteMulty(this.selectedIds.join(',')).then(() => {
                    xmview.showTip('success', '操作成功')
