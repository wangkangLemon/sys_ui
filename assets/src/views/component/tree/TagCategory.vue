<!--标签分类-->
<template>
    <el-tree v-loading="loading" :data="data" :expand-on-click-node="false" @node-click="handleNodeClick" @node-expand="handleNodeExpand" ref="container">
    </el-tree>
</template>
...
export default {
    ...
    methods: {
        ...
        handleNodeExpand(data, node, nodeDom) {
            ...
            tagCategoryService.getTree({ id: data.value }).then(ret => {
                ret.map((item) => {
        ...
        initData() {
            this.loading = true
            return tagCategoryService.getTree({}).then(ret => {
