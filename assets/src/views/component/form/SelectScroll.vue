...
<template>
    <el-select v-model="selectVal" filterable :placeholder="currPlaceholder" ref="container" @visible-change="handleVisibleChange"
        @change="handleChange" :clearable="true" no-data-text="暂无数据" :disabled="disabled" no-match-text="没有数据">
        <el-option :disabled="true" value="xmystinputval" style="height: 50px">
            <el-input @change="filter" placeholder="搜索内容"></el-input>
        </el-option>
        <el-option v-for="item in data" :label="item.name" :key="item.id" :value="item.id">
        </el-option>
    ...
    export default {
        ...
        data() {
            return {
                ...
                data: this.list == null ? [] : this.list.data,
                loading: false,
