//  ================首页部分=================
// 设置内容上边的网站路径
export const INDEX_SETWEBPATH = 'INDEX_SETWEBPATH'
export const INDEX_SET_NAVMENU = 'INDEX_SET_NAVMENU'
export const INDEX_SET__MENU_ACTIVE = 'INDEX_SET__MENU_ACTIVE'
export const INDEX_SET__CLEARFETCHPARAM = 'INDEX_SET__CLEARFETCHPARAM' // 设置是否需要清空筛选条件
export const INDEX_SET__SETSECMENU = 'INDEX_SET__SETSECMENU' // 设置是否需要清空筛选条件
export const INDEX_SET__SETSECPID= 'INDEX_SET__SETSECPID' // 设置是否需要清空筛选条件
export const INDEX_SET__SAVEGOVRANK = 'INDEX_SET__SAVEGOVRANK' // 设置保存部门层级关系的数据
export const INDEX_SET__SAVEEXAMCATEGORY = 'INDEX_SET__SAVEEXAMCATEGORY' //应试考试栏目存储id
// export const INDEX_SET__SETSAVECOURSECATE= 'INDEX_SET__SETSAVECOURSECATE' // 设置是否需要清空筛选条件
